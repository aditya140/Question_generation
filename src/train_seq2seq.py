...
if __name__ == "__main__":
    ...
    ques_train, ques_test, ans_train, ans_test = train_test_split(
    ...
    for ep in range(EPOCHS):
        ...
        if HYPERDASH:
            exp.metric("EPOCH", ep)
        train_loss = train(
            seq2seq, train_dataloader, optimizer, loss_func, device, verbose=True
        )
        ...
        print(
            f"\tTime per Epoch = {int(time_taken/60)} mins {int(time_taken) - int(time_taken/60)*60}\n"
        )
