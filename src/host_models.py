...
version_selected = st.sidebar.slider('Select version',min_value=min(version),max_value=max(version))
model,inpLang,optLang=load_st_model(model_name,version_selected)
...
if dec_type=="Greedy":
    st.text(" ".join(decoder.greedy(txt,max_len=max_length)))
elif dec_type=="Beam":
