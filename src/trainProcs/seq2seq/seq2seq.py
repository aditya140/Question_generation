...
for ep in range(hp.epochs):
    ...
    print(f"\tTraining Loss : {np.mean(train_loss)}",f"Val Perplexity : {math.exp(np.mean(train_loss))}",sep="\t|\t")
    print(f"\tVal Loss      : {val_loss}"           ,f"Val Perplexity : {math.exp(val_loss)}",sep="\t|\t")
