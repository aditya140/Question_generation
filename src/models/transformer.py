...
class EncoderLayer(nn.Module):
    def __init__(
        self, hid_dim, n_heads, pf_dim, dropout,
    ):
        super().__init__()
...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, hid_dim, n_heads, dropout):
        ...
        self.scale = Parameter(
            torch.sqrt(torch.FloatTensor([self.head_dim])), requires_grad=False
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.dropout = nn.Dropout(dropout)
        self.scale = Parameter(
            torch.sqrt(torch.FloatTensor([hid_dim])), requires_grad=False
        )
...
class Encoder(nn.Module):
    def __init__(
        self, input_dim, hid_dim, n_layers, n_heads, pf_dim, dropout, max_length=100
    ):
        super().__init__()
...
class transformer(nn.Module):
    ...
    def make_src_mask(self, src):
        src_mask = (
            (src != self.src_pad_idx).unsqueeze(1).unsqueeze(2).to(self.template.device)
        )
        return src_mask

    def make_trg_mask(self, trg):
        trg_pad_mask = (
            (trg != self.trg_pad_idx).unsqueeze(1).unsqueeze(2).to(self.template.device)
        )
        trg_len = trg.shape[1]
        trg_sub_mask = (
            torch.tril(torch.ones((trg_len, trg_len))).bool().to(self.template.device)
        )
    ...
    def greedy(self, src, start_token, stop_token, max_len=100):
        ...
        enc_src = self.encoder(src, src_mask)
        trg_tensor = (
            torch.tensor(start_token).unsqueeze(0).unsqueeze(0).to(self.template.device)
        )
        stop = False
        while not stop:
    ...
    def beam(self, src, start_token, stop_token, beam_width=3, max_len=100):
        ...
        stop_token = torch.tensor(stop_token).unsqueeze(0).to(self.template.device)
                    ...
                    for i in range(beam_width):
                        ...
                        new_beam.add(
                            ...
                            sequence=torch.cat(
                                [
                                    seq,
                                    out.indices[:, -1][0][i].unsqueeze(0).unsqueeze(0),
                                ],
                                dim=1,
                            ),
                            hidden=None,
